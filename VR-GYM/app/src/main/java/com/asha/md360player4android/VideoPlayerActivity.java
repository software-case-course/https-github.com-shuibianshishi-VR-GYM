...
public class VideoPlayerActivity extends MD360PlayerActivity {
    ...
    public static float speed=1.0f;//速度               2017/3/15
    ...
    private void changeSpeed(){
        new Thread(new Runnable() {
            ...
            public void run() {
                while(true){
                    if (BaseActivity.interval<3&&speed<5.0f){
                        if (ISPAUSE)
                    ...
                    } else if (BaseActivity.interval>3&&BaseActivity.interval<6&&speed>0){
                        speed=speed-1.0f;
                        mMediaPlayerWrapper.setSpeed(speed);
                        Log.d(TAG, "speed: "+speed);
                    }else if (BaseActivity.interval>10&&BaseActivity.interval<6&&speed>0){
                        speed=1.0f;
                        mMediaPlayerWrapper.setSpeed(speed);
        ...
        bdata.start(new ValueEventListener() {
            ...
            public void onDataChange(JSONObject jsonObject) {
                if (BmobRealTimeData.ACTION_UPDATEROW.equals(jsonObject.optString("action"))){
                    Log.d(TAG, "onDataChange: aaaaaaaaaaaaaaaa");
                }
            }
        });
    }
